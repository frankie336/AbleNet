...
def CreateDevice(query,commit,HostName,ManagementIpAddress,ServiceType,
             ModelName,SiteName):
    ...
    if query == True:
        ...
        stmt = select(Device).where(Device.HostName == 'bas01PE01')
...
def DeviceTableInteract(query,commit):
    ...
    CreateDevice(query,commit,HostName,
                 ManagementIpAddress,
                 ServiceType,ModelName,SiteName)


DeviceTableInteract(query=True, commit=False)
